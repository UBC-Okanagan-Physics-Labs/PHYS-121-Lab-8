...
###############################################################################
# For printing outputs in bold & colour (copied from Stack Overflow)          #
# - modified 20240324                                                         #
############################################################################### 
# Start the 'cstr' function.
...
###############################################################################
# Message to remind students to save their work before exporting              #
# - modified 20240324                                                         #
############################################################################### 
# Start the 'saveMessage' function.
def saveMessage():
    print('\n')
    display(html_print(cstr2("\nSTOP! Save your work before running running the submission cell with 'grader.export(...)'.\n", color = 'red')))


###############################################################################
# Message to remind students to wait before downloading their zip file        #
# - modified 20240324                                                         #
############################################################################### 
# Start the 'waitMessage' function.
def waitMessage():
    display(html_print(cstr2("Your zip file is being generated and populated with the correct files.\n", color = 'red')))
    display(html_print(cstr2("This process will only take a few moments, please be patient.\n", color = 'red')))
    display(html_print(cstr2("When things are ready, a download link and a second message will appear below.\n", color = 'red')))
    display(html_print(cstr2("You MUST use the provided link to download your submission.\n", color = 'red')))
    display(html_print(cstr2("Failure to follow these instructions may result in a corrupt submission and a grade of zero.\n", color = 'red')))
    display(html_print(cstr2("------------------------------------------------------------------------------------\n", color = 'red')))


###############################################################################
# Message to let students know that it is safe to proceed                     #
# - modified 20240324                                                         #
############################################################################### 
# Start the 'proceedMessage' function.
def proceedMessage():
    import glob
    import zipfile
     
    display(html_print(cstr2("------------------------------------------------------------------------------------\n", color = 'blue')))
    display(html_print(cstr2("Thank you for waiting.\n", color = 'blue')))
    display(html_print(cstr2("Your zip file is now ready to be downloaded.\n", color = 'blue')))
    display(html_print(cstr2("Please click on the 'here' link above to download the zip file.\n", color = 'blue')))
    display(html_print(cstr2("To complete your submission, please upload the zip file to Canvas without making any changes to its name or contents.\n", color = 'blue')))
    ...
    for x in files:
        try:
            the_zip_file = zipfile.ZipFile(x)
            ret = the_zip_file.testzip()
            print('\n', x, 'contains the following files:\n')
            print(*the_zip_file.namelist(), sep = '\n')
        except:
            print(x, 'may be corrupt')